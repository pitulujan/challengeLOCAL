...
class VectorDB:
    ...
    def batch_index_movies(self, movies: List[Dict[str, Any]]) -> None:
        """Batch index multiple movies in Typesense."""
        try:
            for movie in movies:
                if "id" not in movie or not movie["id"]:
                    raise ValueError(f"Movie document must contain a valid 'id' field (UUID): {movie}")
            self.client.collections[self.collection_name].documents.import_(movies, {"action": "upsert"})
        except Exception as e:
            print(f"Failed to batch index movies: {e}")
